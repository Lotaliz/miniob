                   ...
                   span<const AttrInfoSqlNode> attributes, StorageFormat storage_format)
{
  ...
  if (trx_fields != nullptr) {
    ...
    for (size_t i = 0; i < trx_fields->size(); i++) {
      ...
      fields_[i] = FieldMeta(field_meta.name(), field_meta.type(), field_offset, field_meta.len(), false /*visible*/, false /*nullable*/, field_meta.field_id());
  ...
  for (size_t i = 0; i < attributes.size(); i++) {
    ...
    rc = fields_[i + trx_field_num].init(
      attr_info.name.c_str(), attr_info.type, field_offset, attr_info.length, true /*visible*/, attr_info.nullable, i);
    if (OB_FAIL(rc)) {
