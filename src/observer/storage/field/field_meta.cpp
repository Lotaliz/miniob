...
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, nullable, field_id);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool nullable, int field_id)
{
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  return field.init(name, type, offset, len, visible, nullable, field_id);
}
